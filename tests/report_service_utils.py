...
def generate_report(dataset_id: int, report_type: str, token: str) -> int:
    r = requests.post(
        ...
        + f"/api/v1/openagri-report/{report_type}/dataset/{dataset_id}",
        headers={"Authorization": f"Bearer {token}"},
        timeout=5,
    )
...
def download_report(report_id: int, token: str, report_type=None):
    ...
    report_name = (
        f"report_{report_id}_{report_type}.pdf"
        if report_type
        else f"report_{report_id}"
    )
...
def generate_reports_for_dataset(
    dataset_id: str, types: list, token: str, isDownload: bool = True
):
